# Main script logic
$samples = @{}
$baseDir = $PWD.Path

# Drum abbreviation mapping
$drumMap = @{
    bd = @("kick", "bassdrum", "bd", "kick1", "kick5")
    sd = @("snare", "snaredrum", "sd")
    rim = @("rim", "rimshot")
    cp = @("clap", "handclap", "TR707_CLAP")
    hh = @("closedhat", "hihat_closed", "hh", "808hh", "ch", "wa_808tape_closedhat_08_clean")
    oh = @("openhat", "hihat_open", "oh", "505_OPEN_HH", "hho")
    cr = @("crash", "wa_808tape_crash_03_sat")
    rd = @("ride")
    ht = @("hightom", "tomtom_high", "ht", "tomhi")
    mt = @("midtom", "tomtom_mid", "mt")
    lt = @("lowtom", "tomtom_low", "lt", "tomlo")
    agogo_high = @("agogo_high")
    agogo_low = @("agogo_low")
    bongo_mute = @("bongo_mute")
    bongo_rim = @("bongo_rim")
    bongo = @("bongo")
    cabasa = @("cabasa")
    conga_high = @("conga_high")
    conga_low = @("conga_low")
    conga_mute = @("conga_mute")
    conga_rim = @("conga_rim")
    cowbell = @("cowbell")
    handclap = @("handclap")
    hihat_pedal = @("hihat_pedal")
    maracas = @("maracas")
}

# Function to sanitize names for JSON keys
function Sanitize-Name($name) {
    return $name.ToLower() -replace '[kb6]', '' -replace '[^a-z0-9]', ''
}

# Function to process a set of files for an instrument
function Process-Instrument($files, $baseDir, $drumMap) {
    $drumKit = @{}
    $filePaths = @()
    $isDrumKit = $false

    foreach ($file in $files) {
        $fileNameLower = $file.Name.ToLower()
        $found = $false
        foreach ($drumKey in $drumMap.Keys) {
            foreach ($pattern in $drumMap[$drumKey]) {
                if ($fileNameLower -like "*$pattern*") {
                    $relativePath = $file.FullName.Substring($baseDir.Length + 1).Replace('\', '/')
                    $drumKit[$drumKey] = $relativePath
                    $found = $true
                    $isDrumKit = $true
                    break
                }
            }
            if ($found) { break }
        }
    }

    if ($isDrumKit) {
        return $drumKit
    } else {
        # Not a drum kit, return a simple list of files
        return $files | ForEach-Object { $_.FullName.Substring($baseDir.Length + 1).Replace('\', '/') }
    }
}

# Get top-level directories, excluding .git
$topLevelDirs = Get-ChildItem -Path $baseDir -Directory | Where-Object { $_.Name -ne ".git" }

foreach ($dir in $topLevelDirs) {
    $subDirs = Get-ChildItem -Path $dir.FullName -Directory
    
    # Process files in the root of the top-level directory first
    $filesInRoot = Get-ChildItem -Path $dir.FullName -Include @("*.wav", "*.WAV") -File
    if ($filesInRoot.Count -gt 0) {
        $instrumentName = Sanitize-Name($dir.Name)
        $samples[$instrumentName] = Process-Instrument $filesInRoot $baseDir $drumMap
    }

    # Process subdirectories
    if ($subDirs.Count -gt 0) {
        foreach ($subDir in $subDirs) {
            $instrumentName = Sanitize-Name($subDir.Name)
            $filesInSubDir = Get-ChildItem -Path $subDir.FullName -Recurse -Include @("*.wav", "*.WAV") -File

            if ($filesInSubDir.Count -eq 0) {
                continue
            }
            
            $processedInstrument = Process-Instrument $filesInSubDir $baseDir $drumMap
            
            # If a key for the instrument already exists and both are hashtables, merge them
            if ($samples.ContainsKey($instrumentName) -and $samples[$instrumentName] -is [hashtable] -and $processedInstrument -is [hashtable]) {
                foreach ($key in $processedInstrument.Keys) {
                    $samples[$instrumentName][$key] = $processedInstrument[$key]
                }
            } else {
                $samples[$instrumentName] = $processedInstrument
            }
        }
    }
}

# Final processing to handle single vs multiple samples
$finalSamples = @{}
foreach ($key in $samples.Keys) {
    if ($samples[$key] -is [hashtable]) {
        $finalSamples[$key] = $samples[$key]
    } elseif ($samples[$key].Count -eq 1) {
        $finalSamples[$key] = $samples[$key][0]
    } else {
        $finalSamples[$key] = $samples[$key]
    }
}

$finalSamples['_base'] = 'https://raw.githubusercontent.com/warsang/Samples/main/'

$finalSamples | ConvertTo-Json -Depth 10 | Out-File -Encoding utf8 strudel.json